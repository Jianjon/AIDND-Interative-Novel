...
const ModeSelectView = ({
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full max-w-6xl mx-auto p-4 md:p-8 animate-in fade-in duration-500 overflow-y-auto relative">
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                ...
                <div className="bg-slate-900/40 p-6 rounded-xl border border-slate-700 backdrop-blur-sm relative overflow-hidden group hover:border-slate-600 transition-colors">
                    ...
                    <h3 className="text-xl font-bold text-amber-500 mb-6 flex items-center gap-3 font-tome-header border-b border-slate-700/50 pb-3">
                        ...
                        <span>
                            {gameMode === GAME_MODES.NOVEL ? "文風設定 (TONE)" : "裁判風格 (ARBITER)"}
                        </span>
                    </h3>
                        ...
                        {(gameMode === GAME_MODES.NOVEL ?
                            ...
                            [
                                {
                                    id: 'normal',
                                    label: '正統奇幻 (EPIC)',
                                    desc: '標準的 D&D 奇幻風格。英雄史詩，命運與榮耀。'
                                },
                                {
                                    id: 'dark',
                                    label: '黑暗冷酷 (GRIM)',
                                    desc: '強調世界殘酷與心理恐懼。克蘇魯式的壓抑與絕望感。'
                                },
                                {
                                    id: 'humorous',
                                    label: '輕鬆幽默 (WITTY)',
                                    desc: '泰瑞·普萊契風格。充滿機智對話與諷刺的荒誕冒險。'
                                }
                            ]
                            :
                            // TRPG Arbiter Styles
                            [
                                {
                                    id: 'arbiter',
                                    label: '公正嚴明 (Fair)',
                                    desc: '嚴格遵守 5E 規則。不放水，不偏袒，強調戰術挑戰。'
                                },
                                {
                                    id: 'pulpy',
                                    label: '爽快英雄 (Heroic)',
                                    desc: '規則為輔，帥氣為主。亦即「酷炫法則 (Rule of Cool)」。'
                                },
                                {
                                    id: 'murderhobo',
                                    label: '極限殺戮 (Hardcore)',
                                    desc: '怪物極具侵略性。強調生存壓力，適合老手挑戰。'
                                }
                            ]
                        ).map(opt => (
                            <button
                            ...
                            >
                                <div className={`w-4 h-4 mt-1 rounded-full border-2 flex items-center justify-center
                                    ${gameOptions.tone === opt.id ? 'border-amber-500 bg-amber-500/20' : 'border-slate-600'} `}>
                                    {gameOptions.tone === opt.id && <div className="w-2 h-2 rounded-full bg-amber-500" />}
                                </div>
                                <div>
                                    <div className={`font-bold font-tome-header text-sm mb-1 uppercase tracking-wider ${gameOptions.tone === opt.id ? 'text-amber-100' : 'text-slate-300'} `}>
                                        {opt.label}
                                    </div>
                                    <div className="text-xs text-slate-400 leading-relaxed font-serif opacity-80">
                                        {opt.desc}
            ...
            <button
                onClick={startGame}
                className="w-full max-w-xl py-6 rounded-xl bg-gradient-to-r from-amber-700 to-amber-600 hover:from-amber-600 hover:to-amber-500 text-white shadow-xl shadow-amber-900/30 transition-all hover:scale-[1.02] active:scale-[0.98] font-bold text-xl tracking-widest uppercase flex items-center justify-center gap-3 group relative overflow-hidden"
            >
                ...
                <span className="relative z-10 flex items-center gap-2">
                    開始冒險 (START ADVENTURE) <ChevronRight size={24} />
                </span>
            </button>
        </div>
    );
